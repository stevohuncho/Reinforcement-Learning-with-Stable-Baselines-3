...
from transfer_learning.map_resizers.frozen_lake_resizer import FrozenLakeResizer
from transfer_learning.utils import get_map_from_rgb_array, get_size_from_model
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.training_model_map, self.training_model_map_size = get_map_from_rgb_array(env.render())
        self.pretrained_model_size = get_size_from_model(self.pretrained_model)
        self.map_resizer = FrozenLakeResizer(self.training_model_map, self.pretrained_model_size)
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            if self.use_pretrained:
                env = FrozenLakeEnv(
                    ...
                    fps=10,
                    desc=self.map_resizer.convert_map()
                ).dummy_vec_env(1)
                self.pretrained_model.set_env(env)
                obs = env.reset()
                unscaled_action, _ = self.pretrained_model.predict(obs, deterministic=False)
            else:
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            if rewards[0] == env.metadata['hole_reward']:
